...
y_min = min(error_distances)

""" Add vertical lines to show the different pertubation_types."""
g0_0.vlines(np.array(collect_attempts) - 0.5, ymin = y_min - 1, ymax = y_max + 1, color = 'blue', alpha = 0.5)
""" Add labels to the vertical lines."""
for i in range(len(collect_attempts)):
    g0_0.text(collect_attempts[i] - 1.5, y_min + 0.5, string_attempts[i], rotation = 90, color = 'blue', alpha = 0.5)
...
""" Save the plots."""
g0.figure.savefig('error_angles_{}.svg'.format(subject_name))
g1.figure.savefig('error_angles_over_pertubation_types_{}.svg'.format(subject_name))
...
df0.to_csv('error_angles_{}.csv'.format(subject_name), header=True, index=True,)
df0_0.index.name = 'index'
df0_0.to_csv('error_distances_{}.csv'.format(subject_name), header=True, index=True,)
df1.index.name = 'index'
df1.to_csv('error_angles_over_pertubation_types_{}.csv'.format(subject_name), header=True, index=True,)
