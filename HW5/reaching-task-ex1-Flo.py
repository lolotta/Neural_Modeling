...
while running:
    ...
    # Design experiment
    a1, a2, a3, a4, a5, a6 = 0, 4, 8, 12, 16, ATTEMPTS_LIMIT
    if attempts == a1:
        pertubation_mode = False
    elif attempts == a2:
        pertubation_mode = True
    ...
    if check_target_reached():
        ...
        error_distance = np.sqrt(((target_pos[1] - circle_pos[1])**2) + ((target_pos[0] - circle_pos[0])**2))
        ...
        target_starting_distance = ((target_pos[1] - START_POSITION[1]))#**2

        error_angle = math.degrees(math.sin(error_distance/ target_starting_distance)) #+ 180
    ...
    #miss if player leaves the target_radius + 1% tolerance
        ...
        # CALCULATE AND SAVE ERRORS between target and circle end position for a hit
        """ Calulate the euclidean distance between the target and the circle end position"""
        error_distance = np.sqrt(((target_pos[0] - circle_pos[0])**2) + ((target_pos[1] - circle_pos[1])**2))
        error_distances.append(error_distance)
        
        """ Get from the target_pos and circle_pos the angle. """
        error = target_pos[0] - circle_pos[0], target_pos[1] - circle_pos[1]
        """ Get the angle of the error vector. """
        target_starting_distance = ((target_pos[1] - START_POSITION[1]))
        error_angle = math.degrees(math.sin(error_distance/ target_starting_distance)) #+ 180
        if target_pos[0] < circle_pos[0]:
            error_angle = error_angle * -1
        if move_faster:
...
g0.vlines(np.array(collect_attempts) - 0.5, ymin = -20, ymax = 20, color = 'blue', alpha = 0.5)
""" Add labels to the vertical lines."""
for i in range(len(collect_attempts)):
    g0.text(collect_attempts[i] - 1.5, 10, string_attempts[i], rotation = 90, color = 'blue', alpha = 0.5)
